...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    concat: {
      rb: {
        ...
        files: {
          '.compass/lib/wunderfront.rb': ['.compass/.template/extension.rb']
        }
      },
      gemspec: {
        options: {
        ...
        files: {
          '.compass/wunderfront.gemspec': ['.compass/.template/extension.gemspec']
        }
      }
    },
    ...
    exec: {
      ...
      ext: {
        cmd: 'cd .compass && bundle exec gem build wunderfront.gemspec && mv wunderfront-' + userConfig.client.version + '.gem ../wunderfront-' + userConfig.client.version + '.gem && cd ..'
      },
      install: {
        cmd: 'gem install wunderfront-' + userConfig.client.version + '.gem && rm wunderfront-' + userConfig.client.version + '.gem'
      },
      bundle: {
        cmd: function(path) {
          if (path === '.') {
  ...
  grunt.registerTask('extension', 'Build your Compass Extension', function() {
    ...
    gemfile += "\ngem 'WunderFront', '~>" + grunt.userConfig.client.version + "'";
